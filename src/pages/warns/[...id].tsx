...
export default function WarnInfo({ id }: { id: number }) {
    ...
    useEffect(() => {
        if (id != 0) {
            ...
            fetch(`/api/warns?id=${id}`, { signal: controller.signal })
                .then((res) => res.json())
                .then((data) => {
                    setData(data)
                    setLoading(false)
    ...
    return (
        <>
        <Head>
            <title>{ (isLoading ? `Pending...` : isError ? "Error" : data.data != null ? `Warn #${id}` : "Not found") + ' | Punishments / RoseMC'}</title>
        </Head>
