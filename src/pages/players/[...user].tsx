...
import { PlayerProfile } from "src/components/Table";
...
export default function BanInfo({ user, page }: { user: string, page: number }) {
    ...
    useEffect(() => {
        if (user != "") {
            ...
            fetch(`/api/profile?user=${user}&page=${page}`, { signal: controller.signal })
                .then((res) => res.json())
                .then((data) => {
                    setData(data.query)
                    setLoading(false)
    ...
    return (
        <>
        <Head>
            <title>{ (isLoading ? `Pending...` : isError ? `Error` : data == null ? "Not found" : `${data.user}'s Profile`) + ' | Punishments / RoseMC'}</title>
        </Head>
