...
export default function BanList({ page }: { page: number }) {
    ...
    useEffect(() => {
        ...
        fetch(`/api/bans?page=${page}`, { signal: controller.signal })
            .then((res) => res.json())
            .then((data) => {
                setData(data)
                setLoading(false)
    ...
    <div className="content">
        ...
        <div className="card center" style={{maxWidth: '900px', marginTop: '20px'}}>
            ...
            <div style={{marginBottom: '16px'}}>
                <h3 style={{fontFamily: 'Minecraft', marginBottom: '0px'}}>List for Bans</h3>
            ...
            <div style={{marginBottom: '16px', overflow: isLoading ? 'none' : 'auto'}}>
                { isLoading ? <span className="loader"></span> : isError ? <h6>An error occured while fetching data. Please try again.</h6> : <PunishTable type="bans" data={data} />}
            </div>
            <div>
                {
                    (!isLoading && !isError) ? (
